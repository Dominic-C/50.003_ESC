  ...
  <v-container fluid fill-height class="ds-calendar-container">
    <v-layout>
      <v-flex xs12>
          ...
          <v-layout align-center justify-center>
            ...
            <div>
              <slot name="summary" v-bind="{summary, calendar}">
                <h1 class="title ds-light-forecolor" style="margin-bottom:0">{{ summary }}</h1>
              </slot>
            </div>
        ...
        <slot
          name="weeklyCalendarEventDialog"
        ...
        <slot
          name="weeklyCalendarConfirmPrompt"
        ...
        >
          <v-dialog ref="promptDialog" v-model="promptVisible" v-bind="promptDialog">
            <v-card>
              ...
              <v-card-actions>
                <v-btn color="primary" flat @click="choosePrompt( true )">{{ labels.promptConfirm }}</v-btn>
                <v-spacer></v-spacer>
                <v-btn
                  color="secondary"
                  flat
  ...
  methods: {
    ...
    add(day) {
      ...
      if (useDialog) {
        ...
        if (this.suggestible) {
          eventDialog.addSchedule(day, Schedule.forDay(day), {
    ...
    addAt(dayHour) {
      ...
      if (useDialog) {
        //fill in suggestedBy and locked status by default for suggestible mode
        if (this.suggestible) {
          eventDialog.addSchedule(
            dayHour.day,
            Schedule.forTime(dayHour.day, dayHour.hour),
            { suggestedBy: this.username, locked: false }
          );
        } else if (this.requestable) {
          eventDialog.addSchedule(
            dayHour.day,
            Schedule.forTime(dayHour.day, dayHour.hour),
            { requestedBy: this.username, locked: false }
          );
        } else {
