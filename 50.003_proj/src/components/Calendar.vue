...
<div>
  <v-app id="dayspan" v-cloak class="ds-calendar-app" ref="app">
    ...
    <v-toolbar app flat fixed class="ds-app-calendar-toolbar" color="white">
    ...
    <v-content class="ds-expand">
      <v-container fluid fill-height class="ds-calendar-container">
        ...
        <ds-gestures @swipeleft="next" @swiperight="prev">
          ...
          <div v-if="currentType.schedule" class="ds-expand">
            ...
            <slot name="calendarAppAgenda" v-bind="{$scopedSlots, $listeners, calendar, add, edit, viewDay}">
              ...
              <ds-agenda v-bind="{$scopedSlots}" v-on="$listeners" :calendar="calendar" @add="add" @edit="edit"
          ...
          <div v-else class="ds-expand">
            ...
            <slot name="calendarAppCalendar" v-bind="{$scopedSlots, $listeners, calendar, add, addAt, edit, viewDay, handleAdd, handleMove}">
              ...
              <ds-calendar ref="calendar" v-bind="{$scopedSlots}" v-on="$listeners" :calendar="calendar" @add="add"
                @add-at="addAt" @edit="edit" @view-day="viewDay" @added="handleAdd" @moved="handleMove"></ds-calendar>
        ...
        <slot name="calendarAppEventDialog" v-bind="{$scopedSlots, $listeners, calendar, eventFinish}">
          ...
          <ds-event-dialog ref="eventDialog" v-bind="{$scopedSlots}" v-on="$listeners" :calendar="calendar" @saved="eventFinish"
            @actioned="eventFinish">
			
			<template slot="eventPopoverBodyTop"  v-bind="{$scopedSlots}" v-on="$listeners" :calendar="calendar" @saved="eventFinish">
